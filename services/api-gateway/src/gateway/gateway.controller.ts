...
import { JwtAuthGuard, LoggerService } from '@bazos/shared';
...
export class GatewayController {
   ...
   */
  @Get('bazos/oauth/callback')
  async bazosOAuthCallback(@Req() req: ExpressRequest, @Res() res: ExpressResponse) {
    const path = req.url.replace('/api/bazos', '');
  ...
  /**
   * Route bazos requests (requires auth)
   */
  @All('bazos/*')
  @UseGuards(JwtAuthGuard)
  async bazosRoute(@Req() req: ExpressRequest, @Res() res: ExpressResponse) {
    const path = req.url.replace('/api/bazos', '');
    return this.routeRequest('bazos', `/bazos${path}`, req, res);
  }
  ...
  async apiRoot(@Req() req: ExpressRequest, @Res() res: ExpressResponse) {
    ...
    res.status(404).json({
      ...
      error: {
        code: 'NOT_FOUND',
        message: `Cannot ${req.method} ${req.url}. Available endpoints: /api/auth/*, /api/bazos/*, /api/import/*, /api/settings/*`,
      },
      path: req.url,
      originalUrl: req.originalUrl,
