...
import { LoggerService } from '@bazos/shared';
...
export class GatewayService implements OnModuleInit {
  ...
  constructor(
    ...
    this.serviceUrls = {
      bazos: getServiceUrl('BAZOS_SERVICE_URL', 'BAZOS_SERVICE_PORT', 'bazos'),
      import: getServiceUrl('IMPORT_SERVICE_URL', 'IMPORT_SERVICE_PORT', 'import'),
      settings: getServiceUrl('SETTINGS_SERVICE_URL', 'BAZOS_SETTINGS_SERVICE_PORT', 'settings'),
  ...
  private async warmupServiceConnection(serviceName: string, baseUrl: string): Promise<void> {
    ...
    if (baseUrl.includes('gateway-proxy')) {
      // Nginx routes: /bazos/*, /settings, /import/*
      if (serviceName === 'bazos') {
        healthUrl = `${baseUrl}/bazos/health`;
  ...
  async forwardRequest(
    ...
    const isValidationOperation = path.includes('/validate/bazos') || path.includes('/validate/');
    ...
    const isInternalService = ['bazos', 'import', 'settings'].includes(serviceName);
