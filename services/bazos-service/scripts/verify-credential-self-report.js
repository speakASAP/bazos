...
check('probes role-scoped routes, never /health', () => {
  ...
  assert(text.includes('/api/stock/'), 'does not probe the warehouse stock route');
...
check('both principals match auth exactly', () => {
  const text = fs.readFileSync(path.join(root, 'src/health/credential-self-reporter.ts'), 'utf8');
  for (const p of [
    'svc-bazos-service--orders-microservice@internal.alfares.cz',
    'svc-bazos-service--warehouse-microservice@internal.alfares.cz',
  ]) {
