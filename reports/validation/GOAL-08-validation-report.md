...
- Static source scan for local credential form/proxy markers in `services/aukro-service/src/ui`: passed; no matches for `/ui/auth/login`, `/ui/auth/register`, `<form id="login-form"`, `type="password"`, `fetch(authMode`, `data-auth-tab`, or `auth-submit`.
