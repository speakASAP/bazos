...
2026-06-30: Bazos Warehouse stock-event orchestration completed in source on `main`. RabbitMQ `stock.updated` now uses Warehouse `available` for linked Bazos ads; `stock.out` forces quantity `0` and removes ads from the local sale surface via `isActive=false` and `publishStatus=deleted`. Durable per-ad evidence is recorded under `lastPolicyCheck.warehouseStockSync`, idempotent replays no-op, failures are logged and persisted where possible, Warehouse is not mutated, and default outbound channel-state pacing is 1 request/sec. Validation so far: focused RabbitMQ stock-event spec pass and shared build pass. Report: reports/validation/BAZOS-WAREHOUSE-STOCK-GATE-2026-06-29.md.
...
```text
2026-06-30: Bazos Warehouse stock-event orchestration completed in source on `main`. Added Warehouse `stock.updated`/`stock.out` RabbitMQ projection for linked Bazos ads with durable `lastPolicyCheck.warehouseStockSync` evidence, idempotent replay handling, failure recording/logging, no Warehouse mutation, and default 1 request/sec pacing. Validation: focused RabbitMQ stock-event spec pass; shared build pass; remaining gates recorded in the final session/deployment report.
