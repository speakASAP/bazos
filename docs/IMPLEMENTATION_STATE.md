...
```text
2026-06-26: Goal 10 legacy Aukro publishing removal completed in source on `main`. Removed `services/aukro-service/src/aukro/publishing/*`, removed `PublishingModule` imports from `AukroModule` and `OffersModule`, removed legacy publish policy/enqueue endpoints from `OffersController`, and routed UI monitoring/ad/policy/publish calls to modern `/api/bazos/*` endpoints. Validation: `npm test` pass (5 suites, 82 tests); `npm --prefix shared run build` pass; `npm --prefix services/aukro-service run build` pass; `git diff --check` pass; source legacy-reference scan pass. Reports: implementation-goals/GOAL-10-remove-legacy-aukro-publishing.md, reports/validation/GOAL-10-validation-report.md, reports/validation/GOAL-10-intent-compliance-report.md. Production deployment recorded separately after rollout.
