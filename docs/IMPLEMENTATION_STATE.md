...
```text
2026-06-26: Goal 10 legacy Aukro publishing removal completed and deployed. Source commit `f7a60e6` removed `services/aukro-service/src/aukro/publishing/*`, removed `PublishingModule` imports from `AukroModule` and `OffersModule`, removed legacy publish policy/enqueue endpoints from `OffersController`, and routed UI monitoring/ad/policy/publish calls to modern `/api/bazos/*` endpoints. Validation before deploy: `npm test` pass (5 suites, 82 tests); `npm --prefix shared run build` pass; `npm --prefix services/aukro-service run build` pass; `git diff --check` pass; source legacy-reference scan pass. Deployment image `localhost:5000/bazos-service:f7a60e6`, digest `sha256:9d5f833c5c72b6dd7524b19a8ae272aabc3ac92ec9c432cc7b49e1b3b8dcef53`; production pod `bazos-service-66f7859c6f-7ph6p` Ready 1/1, Running, 0 restarts. Production smoke passed for `/health`, `/`, `/client`, `/ui/app.js`; removed legacy routes `/publishing/limits`, `/publisher-queue`, `/verification-sessions`, and `/publishing-monitoring/summary` returned 404. Follow-up commit `67097bf` fixes modern `/api/bazos/*` unauthenticated responses from 500 to 401 and passed validation, but its rollout was blocked by node-wide Kubernetes `ContainerCreating`; deployment was rolled back to stable `f7a60e6`. Reports: implementation-goals/GOAL-10-remove-legacy-aukro-publishing.md, reports/validation/GOAL-10-validation-report.md, reports/validation/GOAL-10-intent-compliance-report.md, reports/validation/GOAL-10-deployment-report.md.
